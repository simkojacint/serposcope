...
public class MigrationDB extends AbstractDB {
    ...
    protected void upgradeFromV6(Statement stmt) throws Exception {
        ...
        stmt.executeUpdate("alter table `"+QGoogleSearch.TABLE_NAME+"` add column `country` varchar(2) after tld;");
        
        try(Connection con = ds.getConnection(); Statement stmtRO = con.createStatement()){
            ResultSet rs = stmtRO.executeQuery("select * from `"+QGoogleSearch.TABLE_NAME+"`;");
            while(rs.next()){
                GoogleCountryCode country = MigrationTLDtoCountry.fromTld(rs.getString("tld"));
                stmt.executeUpdate("update `"+QGoogleSearch.TABLE_NAME+"` set `country` = '" + country.name() + "' WHERE `id` = " + rs.getInt("id") + ";");
            }            
        }

        stmt.executeUpdate("alter table `"+QGoogleSearch.TABLE_NAME+"` drop column `tld`;");
        stmt.executeUpdate("insert into `"+QConfig.TABLE_NAME+"` values ('app.dbversion','7') on duplicate key update `value` = '7';");
    }    
